...
/// shape. `Str` and `List` exist today; `Hash` and `Set` join their variants
/// as those commands land, at which point the accessors below grow the
/// matching arms.
...
impl StoredValue {
    ...
    /// and in future `hash` / `set`. A missing key is handled by the caller
...
fn get(args: &[Value], storage: &Store) -> Value {
    ...
    match store.get(&key) {
        Some(e) => match &e.value {
            ...
            // GET only understands strings; a list (or any future type) here
            // is a client bug, and Redis answers it with WRONGTYPE.
            StoredValue::List(_) => wrong_type(),
        },
        None => Value::Null,
    }
}
